...
public class GuiTurbineStats extends GuiMekanism
{
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY)
	{
		...
		String stats = LangUtils.localize("gui.mekanism.turbineStats");
		String limiting = EnumColor.DARK_RED + " (" + LangUtils.localize("gui.mekanism.limiting") + ")";
		
		fontRenderer.drawString(stats, (xSize/2)-(fontRenderer.getStringWidth(stats)/2), 6, 0x404040);
		
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.tankVolume") + ": " + tileEntity.structure.lowerVolume, 8, 26, 0x404040);
		...
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.steamFlow"), 8, 40, 0x797979);
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.dispersers") + ": " + tileEntity.structure.clientDispersers + (dispersersLimiting ? limiting : ""), 14, 49, 0x404040);
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.vents") + ": " + tileEntity.structure.vents + (ventsLimiting ? limiting : ""), 14, 58, 0x404040);
		...
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.production"), 8, 72, 0x797979);
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.blades") + ": " + tileEntity.structure.blades + (bladesLimiting ? limiting : ""), 14, 81, 0x404040);
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.coils") + ": " + tileEntity.structure.coils + (coilsLimiting ? limiting : ""), 14, 90, 0x404040);
		...
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.maxProduction") + ": " + MekanismUtils.getEnergyDisplay(rate*energyMultiplier), 8, 104, 0x404040);
		fontRenderer.drawString(LangUtils.localize("gui.mekanism.maxWaterOutput") + ": " + tileEntity.structure.condensers*generators.condenserRate + " mB/t", 8, 113, 0x404040);
